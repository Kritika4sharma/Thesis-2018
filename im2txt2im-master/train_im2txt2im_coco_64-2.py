...
DIR = "/home/kritika/Thesis-Samples-Data-Models/"
...
def main(_):
    ...
    if tf.gfile.IsDirectory(checkpoint_path):
        ...
        checkpoint_path = checkpoint_path + '/model.ckpt-580000'
    ...
    d_loss1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_real_image_logits, labels=tf.ones_like(disc_real_image_logits)))         # <- real image, right text
    ...
    d_loss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_wrong_image_logits, labels=tf.zeros_like(disc_wrong_image_logits)))      # <- wrong image, right text
    ...
    f = open("convergence.txt", "a") ## --by kritika
    # Train txt2im
    ...
    with tf.Session() as sess:
        ...
        num_iter_learnt = ""
        net_c_name = os.path.join(save_dir, 'net_c' + num_iter_learnt + '.npz')
        net_e_name = os.path.join(save_dir, 'net_e' + num_iter_learnt + '.npz')
        net_g_name = os.path.join(save_dir, 'net_g' + num_iter_learnt + '.npz')
        net_d_name = os.path.join(save_dir, 'net_d' + num_iter_learnt + '.npz')
        if True:
            ...
            try:
                ...
                # if step % n_check_step == 0:
                    ...
                    if step % (n_check_step*10) == 0:
                        ...
                        save_images(img_gen, [8, int(batch_size/8)], '{}/train_{:02d}.png'.format(sample_dir, step))  # ---put here by kritika
                        string_out = str(step) + " " + str(total_g_loss/n_check_step) + " " + str(total_d_loss/n_check_step)
                        f.write(string_out)
