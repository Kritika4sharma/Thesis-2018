...
def main_train():
    ...
    for epoch in range(start_epoch, n_epoch+1):
        ...
        for step in range(n_batch_epoch):
            ...
            ## updates D
            errD, optimD = sess.run([d_loss, d_optim], feed_dict={
                            t_real_image : b_real_images,
                            # t_wrong_image : b_wrong_images,
                            t_wrong_caption : b_wrong_caption,
                            t_real_caption : b_real_caption,
                            t_z : b_z})
            ## updates G
            errG, optimG = sess.run([g_loss, g_optim], feed_dict={
                            t_real_caption : b_real_caption,
                            t_z : b_z})

            if(step == n_batch_epoch-1):
                graph_file.write(str(epoch) + " " + str(errG) + " " + str(errD) + " " + str(errRNN) + " " + str(optimG) + " " + str(optimD) + "\n")
